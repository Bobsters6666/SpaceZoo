            ...
            {!isCollapsed && (
                    <div className="flex flex-col gap-[1rem]" >
                        {regionLinks.map(link => 
                            (
                                <Link key={link.title} href={link.link}><p className={ `hover:text-[#84c4f5] hover:scale-105 transition-all duration-300 ${pathName.includes(link.link) ? 'text-[#84c4f5]': ''}`}>{link.title}</p></Link>
                            )
                        )}
                    </div>
                )}
